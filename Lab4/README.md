...
|                | e0/1          | 198.19.1.1     | 255.255.255.252 | Соединение с AS 301 (R21)                    |
...
| **Подсеть**       | **Маска**         | **Назначение**                          | **Пояснение**                                                                 |
|--------------------|-------------------|-----------------------------------------|-------------------------------------------------------------------------------|
| `10.52.255.0/24`  | `255.255.255.0`   | Loopback-интерфейсы                     | Для стабильной идентификации маршрутизаторов в OSPF и BGP.                   |
| `10.52.1.0/30`    | `255.255.255.252` | Соединение R23 ↔ R25                    | Точечное соединение маршрутизаторов.                                         |
| `10.52.1.4/30`    | `255.255.255.252` | Соединение R23 ↔ R24                    | Подсеть для точечного соединения маршрутизаторов.                             |
| `10.52.1.8/30`    | `255.255.255.252` | Соединение R24 ↔ R26                    | Аналогично другим соединениям.                                               |
| `10.52.1.12/30`   | `255.255.255.252` | Соединение R25 ↔ R26                    | Подсеть для точечного соединения маршрутизаторов.                             |
| `192.168.2.0/30`  | `255.255.255.252` | Соединение R25 ↔ R28 (Чокурдах)         | Подключение Чокурдаха к AS 520.                                              |
| `192.168.3.0/30`  | `255.255.255.252` | Соединение R26 ↔ R28 (Чокурдах)         | Соединение маршрутизаторов для Чокурдаха.                                    |
| `198.19.5.0/30`   | `255.255.255.252` | Соединение R24 ↔ AS 2042 (R18)          | Меж-AS соединение с Санкт-Петербургом.                                       |
| `198.19.6.0/30`   | `255.255.255.252` | Соединение R26 ↔ AS 2042 (R18)          | Меж-AS соединение для связи с Санкт-Петербургом.                             |
| `198.19.2.0/30`   | `255.255.255.252` | Соединение R23 ↔ AS 101 (R22)           | Меж-AS соединение с Киторн.                                                  |
| `198.19.4.0/30`   | `255.255.255.252` | Соединение R24 ↔ AS 301 (R21)           | Меж-AS соединение с Ламас.                                                   |
...
| **Устройство** | **Интерфейс** | **IP-адрес**   | **Маска**         | **Назначение**                          |
|----------------|---------------|----------------|-------------------|-----------------------------------------|
| **R23**        | e0/0          | 198.19.2.1     | 255.255.255.252   | Соединение с AS 101 (R22)              |
...
| **R24**        | e0/0          | 198.19.4.1     | 255.255.255.252   | Соединение с AS 301 (R21)              |
...
1. **Loopback-интерфейсы (`10.52.255.0/24`)**:
   - Используются для идентификации маршрутизаторов в OSPF и BGP.
   - Стабильные IP, независимые от физических интерфейсов.

2. **Точечные соединения (`/30`)**:
   - Минимизируют расход адресов и обеспечивают соединение между маршрутизаторами.
   - Используются как для внутренних связей AS, так и для меж-AS соединений.

3. **Меж-AS соединения**:
   - Между AS 520 и соседними автономными системами.
   - Используются выделенные диапазоны (`198.19.x.x`) для эмуляции и тестирования.